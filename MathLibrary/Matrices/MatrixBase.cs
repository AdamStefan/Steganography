...
namespace MathLibrary.Matrices
{
    public class MatrixBase<T>
    {
            ...
            : IEnumerable<MatrixBase<T>>
        {
            ...
            public IEnumerable<MatrixBase<T>> AsEnumerable()
            {
                ...
                while (j < _dataView.RowCount)
                    yield return _dataView.SubMatrix(new Int32Range(j, ++j), new Int32Range(0, _dataView.ColumnCount));
            }
            ...
            : IEnumerable<MatrixBase<T>>
        {
            ...
            public IEnumerable<MatrixBase<T>> AsEnumerable()
            {
                ...
                while (i < _dataView.ColumnCount)
                    yield return _dataView.SubMatrix(new Int32Range(0, _dataView.RowCount), new Int32Range(i, ++i));
            }
        ...
        public MatrixBase(Int32 rows, Int32 columns)
        {
            _dataSource = new MatrixDataSource<T>(rows, columns);
        ...
        public virtual T this[Int32 row, Int32 column]
        {
            get
            {
                ...
                return _dataSource[row + _dataRowRange.Start, column + _dataColumnRange.Start];
            }
            set
            {
                if (!_dataColumnRange.Contains(column + _dataColumnRange.Start)) throw new ArgumentOutOfRangeException("column");
                if (!_dataRowRange.Contains(row + _dataRowRange.Start)) throw new ArgumentOutOfRangeException("row");
                _dataSource[row + _dataRowRange.Start, column + _dataColumnRange.Start] = value;
            }
        }
        ...
        public delegate T ElementPositionalUnaryOperationDelegate(T element, Int32 row, Int32 column);
        public delegate T ElementPositionalBinaryOperationDelegate(T element1, T element2, Int32 row, Int32 column);
        ...
        {
            ElementWiseCopy(this, targetMatrix, targetRowOffset, targetColumnOffset);
        }
        ...
        public virtual void CopyFrom(
            ...
            Int32 targetColumnOffset)
        {
            ElementWiseCopy(sourceMatrix, this, targetRowOffset, targetColumnOffset);
        }
        ...
        {
            return Equality(this, obj as MatrixBase<T>);
        }
        ...
        public virtual MatrixBase<T> SubMatrix(
            ...
            Int32Range columnRange)
        {
            return SubMatrix(this, rowRange, columnRange);
        }
        ...
        {
            SwapColumns(this, column1, column2);
        }
        ...
        public override String ToString()
        {
            ...
            result += String.Format("{0} rows x {1} columns\n", RowCount, ColumnCount) + Environment.NewLine;
            ...
            for (Int32 i = 0; i < RowCount; i++)
            {
                ...
                {
                    rowString += String.Format(" {0} ", this[i, j]);
                }
                result += String.Format("[{0}]" + Environment.NewLine, rowString);
            }
            return result;
        }
        ...
        public static MatrixBase<T> Default(Int32 rows, Int32 columns)
        {
            var defaultMatrix = new MatrixBase<T>(rows, columns);
        ...
        public static void ElementWiseCopy(
            ...
            Int32 targetColumnOffset)
        {
            for (Int32 i = 0; i < source.RowCount; i++)
                for (Int32 j = 0; j < source.ColumnCount; j++)
                    target[i + targetRowOffset, j + targetColumnOffset] = source[i, j];
        }
        ...
        {
            if (IsNull(matrix1)) throw new ArgumentNullException("matrix1");
            if (IsNull(matrix2)) throw new ArgumentNullException("matrix2");
            ...
            var result = new MatrixBase<T>(matrix1.RowCount, matrix1.ColumnCount);
            for (Int32 i = 0; i < result.RowCount; i++)
                for (Int32 j = 0; j < result.ColumnCount; j++)
        ...
        public static MatrixBase<T> ElementWiseOperation(
            ...
            ElementBinaryOperationDelegate operation)
        {
            if (IsNull(matrix)) throw new ArgumentNullException("matrix");
            ...
            MatrixBase<T> matrix2)
        {
            ...
            for (Int32 i = 0; i < matrix1.RowCount; i++)
                for (Int32 j = 0; j < matrix1.ColumnCount; j++)
            ...
            IEnumerable<MatrixBase<T>> matricies)
        {
            ...
            {
                if (IsNull(matrix)) throw new ArgumentNullException();
            ...
            var resultMatrix = new MatrixBase<T>((int)rowCount, totalColumnCount);

            int columnOffset = 0;
            foreach (MatrixBase<T> matrix in matricies)
            {
                ElementWiseCopy(matrix, resultMatrix, 0, columnOffset);
            ...
            IEnumerable<MatrixBase<T>> matricies)
        {
            ...
            var resultMatrix = new MatrixBase<T>(totalRowCount, (int)columnCount);

            int rowOffset = 0;
            foreach (MatrixBase<T> matrix in matricies)
            {
                ElementWiseCopy(matrix, resultMatrix, rowOffset, 0);
